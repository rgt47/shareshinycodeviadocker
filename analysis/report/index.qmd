...
date: "2025-06-28"
categories: [r, shiny, docker, reproducibility]
description: >
...
More formally, this post is the Shiny-specific case study of
the Container layer of the Workflow Construct described in
[post 52](/posts/52-workflow-construct/). The
container-layer fundamentals (Dockerfile authoring, base
image selection, dependency installation, port binding) are
documented in [post 32](/posts/32-sharermdcodeviadocker/);
this post addresses the additional concerns that arise
specifically when the container serves an interactive Shiny
application: process supervision (Shiny Server vs.
ShinyProxy), TCP port exposure, persistent file storage for
user uploads, and reverse-proxy authentication.
